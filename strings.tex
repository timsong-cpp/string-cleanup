...
function or operator \changed{shall have}{has} no other effect \added{on the \tcode{basic_string} object}.
...
\pnum
\effects
Constructs an object \removed{of class \tcode{basic_string}} \added{whose
...
\pnum
\effects
Constructs an object \changed{of class \tcode{basic_string}
and determines its initial string value from}{whose initial value is} the \changed{array of
...
{whose value consists of \tcode{n} copies of \tcode{c}.}
...
\begin{itemdescr}
\pnum
\effects Constructs an object \removed{of class \tcode{basic_string}} \added{whose value is
that of \tcode{str} prior to this call}.
The stored allocator is constructed from \tcode{alloc}. \added{In the second form, \tcode{str} is left in a valid but unspecified state.}
...
equal to \tcode{alloc}. In the second form, \tcode{str} is left in a valid state with an unspecified value.
\end{removedblock}
...
\pnum
\effects
Alters \removed{the length of the string designated by} \added{the value of}
\tcode{*this}
as follows:
...
\tcode{*this}.}{erases the last \tcode{size() - n} elements.}
\item
If
\tcode{n > size()},
\changed{the function replaces the string designated by
...
\begin{itemdescr}
\pnum
\effects \changed{Calls \tcode{push_back(c)};}{Equivalent to: \tcode{return append(size_type\{1\}, c);}}
...
\begin{itemdescr}
\pnum
\effects \removed{Equivalent to: \tcode{return append(basic_string(n, c));}} \added{Appends \tcode{n} copies of \tcode{c} to the string.}
...
\effects
Equivalent to
\tcode{append(\changed{static_cast<size_type>(}{size_type\{}1\added{\}}\removed{)}, c)}.
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\effects Equivalent to: \removed{\tcode{return assign(basic_string(n, c));}}
\begin{addedblock}
\begin{codeblock}
clear();
resize(n, c);
...
\item \tcode{length_error} if \changed{\tcode{size() + n > max_size()}.}{\tcode{n > max_size() - size()}, or}
...
\end{addedblock}
\pnum
\effects \removed{Equivalent to: \tcode{return insert(pos, basic_string(n, c));}} \added{Inserts \tcode{n} copies of \tcode{c} before the character at position \tcode{pos} if \tcode{pos < size()}, or otherwise at the end of the string.}
\end{itemdescr}
...
\pnum
\effects
Inserts a copy of \tcode{c} \changed{before the character referred to by}{at the position} \tcode{p}. 
...
\pnum
\effects
Inserts \tcode{n} copies of \tcode{c} \changed{before the character referred to by}{at the position} \tcode{p}.
...
\tcode{*this}\changed{,
defining a range
\tcode{[first, last)}}{. \range{first}{last} is a valid range}.
...
\pnum
\effects
Equivalent to \tcode{erase(\changed{size}{end}() - 1\removed{, 1})}.
\end{itemdescr}
...
\item \tcode{length_error} if the length of the resulting string would exceed \tcode{max_size()} (see below), or
\item any exceptions thrown by \tcode{allocator_traits<Allocator>::allocate.}
\end{itemize}
\end{addedblock}

\pnum
\effects \removed{Equivalent to: \tcode{return replace(pos1, n1, basic_string(n2, c));}} \added{Determines the effective length \tcode{xlen} of the string to be
removed as the smaller of \tcode{n1} and \tcode{size() - pos1}. If
...
\begin{itemdescr}
\pnum
\changed{\requires}{\expects} \range{begin()}{i1} and \range{i1}{i2} are valid ranges.
...
The program shall not \changed{alter}{modify} any of the values stored in the character array\added{; otherwise, the behavior is undefined}.
\end{itemdescr}
...
\pnum
\changed{\requires}{\remarks}
The program shall not \changed{alter}{modify} the value stored at \tcode{p + size()} 
...
\pnum
\remarks
The expression inside \tcode{noexcept} is equivalent to \tcode{is_nothrow_convertible_v<const T\&,
...
\begin{floattable}{\color{remclr}\tcode{compare()} results}{tab:strings.compare}
{lc}
\topline
...
four use a default constructed allocator. A complete fix to this problem
is deferred to a separate paper, P1165R0. The changes below
only fixes two obvious issues: a pointless extra copy and a bogus
note.}
...
\pnum
\returns
\tcode{\added{std::move(}basic_string<charT, traits, Allocator>(lhs).append(rhs)\added{)}}.
\end{itemdescr}
...
\tcode{std::move(lhs.append(rhs))}.
\begin{note}
\removed{Or equivalently, \tcode{std::move(rhs.insert(0, lhs))}.}
\end{note}
\end{itemdescr}
...
\begin{libefftabvaluenarrow}{\color{remclr}\tcode{basic_string_view(const charT*)} effects}{tab:string.view.ctr.2}
\tcode{data_} & \tcode{str} \\
...
\begin{libefftabvaluenarrow}{\color{remclr}\tcode{basic_string_view(const charT*, size_type)} effects}{tab:string.view.ctr.3}
\tcode{data_} & \tcode{str} \\
\tcode{size_} & \tcode{len} \\
\end{libefftabvaluenarrow}
\end{itemdescr}
...
\begin{addedblock}
\pnum
Let \placeholder{F} be one of \tcode{find}, \tcode{rfind}, \tcode{find_first_of}, \tcode{find_last_of}, \tcode{find_first_not_of}, and \tcode{find_last_not_of}.
\end{addedblock}
\draftnote{Turn the next three paragraphs into a bulleted list:}
\begin{itemize}
\item
Each member function of the form
\begin{codeblock}
